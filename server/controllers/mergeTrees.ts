const mergeTreesExport = (req, res, next) => {
  ...
  class FuncObject {
    constructor(path, filePath, code, ast) {
      ...
      this.funcName =
        this.funcLabel(path) ||
        `anonymous_function_at_${path.node.start}-${path.node.end}_in_${filePath}`; // need oto fix to align better with rest of func naming
      ...
      ast.program.body.forEach((bodyNode) => {
        if (bodyNode.type === "VariableDeclaration") {
          bodyNode.declarations.forEach((declaration) => {
            ...
            declarationObj.definition = code.slice(
              bodyNode.start,
              bodyNode.end
            );
            declarationObj.position = [bodyNode.start, bodyNode.end];
            fileVars[filePath].globalDeclarations[declaration.id.name] =
              declarationObj;
          });
        }
      });
    ...
    listUpdates() {
      ...
      function findOriginalDeclaration(varName, filePath, funcName) {
        ...
        if (varNotAlreadyDeclared) {
          fileVars[filePath].functionLevelDeclarations.forEach(
            (declaration) => {
      ...
      const assignmentChecker = {
        AssignmentExpression(path) {
          ...
          if (path.node.left.type === "MemberExpression") {
            ...
            newUpdateObj.dependentFuncName = newUpdateObj.updateName =
              code.slice(path.node.left.start, path.node.left.end);
            newUpdateObj.dependentFuncDef = newUpdateObj.updateDefinition =
              code.slice(path.node.start, path.node.end);
            newUpdateObj.dependentFuncPosition = newUpdateObj.updatePosition = [
              path.node.start,
              path.node.end,
            ];
            newUpdateObj.originallyDeclared = findOriginalDeclaration(
              newUpdateObj.updateName,
              filePath
            );
          }
      ...
      const updateChecker = {
        UpdateExpression(path) {
          ...
          if (path.node.argument === "MemberExpression") {
            // console.log('dependency - object / memberExpression: ', code.slice(path.node.left.start, path.node.left.end))
            newUpdateObj.dependentFuncName = newUpdateObj.updateName =
              code.slice(path.node.argument.start, path.node.argument.end);
            newUpdateObj.dependentFuncDef = newUpdateObj.updateDefinition =
              code.slice(path.node.start, path.node.end);
            newUpdateObj.dependentFuncPosition = newUpdateObj.updatePosition = [
              path.node.start,
      ...
      path.traverse({
        Identifier(path) {
          ...
          if (
            ...
            // console.log(path)
            let originalDeclaration = findOriginalDeclaration(
              path.node.name,
              filePath,
              funcName
            );
    ...
    for (const method in methodObj) {
      ...
      oneMethod.middlewares.forEach((middleware, i) => {
        ...
        newObj.functionInfo = {
          ...
          funcPosition: [
            middleware.funcInfo.location.start.index,
            middleware.funcInfo.location.end.index,
          ],
